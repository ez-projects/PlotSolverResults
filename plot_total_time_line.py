...
import itertools
markers = itertools.cycle(('^', 'o', '>', 'v',  'H', '<', 'd', 's', 'p','3', 'D',  'x','+', '|', '.',  '*'))
# pudb.set_trace()
# for n in y:
#     plt.plot(x,n, marker = markers.next(), linestyle='')
...
for filename in filenames:
    ...
    for key, value in df.iteritems():
        if key != 'Mesh_Sizes':
            plt.plot(x, df[key], marker=next(markers), label=key, markersize=10.0, linewidth=2.0)
            i += 1
